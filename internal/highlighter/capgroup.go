...
func (cgl *capGroupList) linkedStyle(matches []string, cg capGroup) (fg, bg, style string, ok bool) {
	...
	targetIdx, ok := cgl.index[cg.LinkTo]
	if !ok {
		return "", "", "", false
	}
	...
	for {
		if visited[curIdx] {
			// cycle shouldn't happen due to validation; fail safe
			return "", "", "", false
		}
		visited[curIdx] = true
		...
		// hop if chained link
		if target.LinkTo != "" {
			nextIdx, ok := cgl.index[target.LinkTo]
			if !ok {
				return "", "", "", false
			}
			curIdx = nextIdx
		...
		subIdx := cgl.fullRegExp.SubexpIndex("capGroup" + strconv.Itoa(curIdx))
		sub := ""
		if subIdx >= 0 && subIdx < len(matches) {
			sub = matches[subIdx]
		}

		if len(target.Alternatives) > 0 {
			for _, alt := range target.Alternatives {
				if alt.RegExp.MatchString(sub) {
